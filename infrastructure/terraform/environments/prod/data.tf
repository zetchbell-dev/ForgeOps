# Prod does not instantiate module.ecr (see environments/dev/main.tf and
...
# graph being re-evaluated — prod's plan/apply does not re-run anything in
# modules/ecr, it only reads two output values that were already computed.
